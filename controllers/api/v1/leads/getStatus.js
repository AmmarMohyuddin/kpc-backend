...
async function getStatus(req, res) {
  try {
    ...
    // 4️⃣ Extract statuses from response and filter for Lead type only
    const allStatuses = response.data.items;
    const leadStatuses = allStatuses.filter(
      (status) => status.status_type === "Lead"
    );
    ...
    return successResponse(
      res,
      200,
      "Lead status transitions retrieved successfully",
      leadStatuses
    );
  } catch (error) {
